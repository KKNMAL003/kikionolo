...
// Captures and forwards errors to parent if in an iframe, and logs errors for debugging.
// Usage: Import this file at app entry point to enable global error handling.
// See also: index.ts

// This function will be exported and used by the ErrorBoundary component later
export function sendErrorToIframeParent(
  errorSource: any,
  errorInfo?: { componentStack?: string } | React.ErrorInfo,
) {
  if (
    Platform.OS === 'web' &&
    ...
    if (document.referrer) {
      try {
        ...
        if (webTargetOrigins.includes(referrerURL.origin)) {
          ...
          console.warn(
            `[GlobalErrorHandler] Referrer origin "${referrerURL.origin}" not in webTargetOrigins. Defaulting to '*' for postMessage. Valid targets:`,
            webTargetOrigins,
          );
        }
      } catch (e) {
        console.warn(
          '[GlobalErrorHandler] Could not parse document.referrer or determine target origin:',
          e,
          document.referrer,
        );
      }
      ...
      console.warn(
        '[GlobalErrorHandler] document.referrer is empty. Defaulting to "*" for postMessage. This might happen if the iframe is navigated to directly or due to referrer policy.',
      );
    }
    ...
    try {
      ...
      console.error(
        '[GlobalErrorHandler] Failed to send error to parent via postMessage:',
        postMessageError,
      );
    }
    ...
    if (!window.parent) reason = 'window.parent is null';
      ...
      reason = 'window.parent is the same as window (not in iframe)';
    console.debug(
      `[GlobalErrorHandler] Not sending to parent. Platform: ${Platform.OS}, Reason: ${reason}`,
    );
  }
}
...
if (Platform.OS === 'web' && typeof window !== 'undefined') {
  ...
  window.addEventListener(
    ...
    (event: ErrorEvent) => {
      ...
      if (!error.hasOwnProperty('filename') && event.filename)
  ...
  console.log(
    `[GlobalErrorHandler] Not initializing web error listeners. Platform: ${Platform.OS}, window: ${typeof window}`,
  );
}